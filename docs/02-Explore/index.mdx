...
You will see a view showing all of the servers that make up the environment.  Each of the towers represents a server or some device/appliance that is being monitored remotely.
...
drill into the listener, topics, queues, channels, and more.  Let's focus on the Channels.  Scroll down to the table labeled **"Channels"**.  Select the link next to the one with connection name 127.0.0.1.
...
All of this contextual and relationship data helps you analyze and debug your applications to get to root cause as quickly as possible.  In addition, our 
