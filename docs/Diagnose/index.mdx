...
In this case, only a single Event makes up the Incident, but in many cases, you will see 10 or more **Events** 
that are related to each other contained within a single Incident.
...
You will be taken to an analytics screen where you can see all of the failed calls for **"POST /order"** . 
You can also see latency for the calls is greater than 2 seconds.
